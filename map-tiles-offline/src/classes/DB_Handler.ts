...
export class DB_Handler {
    ...
    onCreate() {
        ...
        // those goes async with promises!!!
        this.pushedSQL = 6
        this.mDB.executeSql(CREATE_STATE_TABLE, {}).then(() => {
            this.checkReadyStatus()
        })
        this.mDB.executeSql(CREATE_TASK_TABLE, {}).then(() => {
            this.checkReadyStatus()
        })
        this.mDB.executeSql(CREATE_ROUTE_TABLE, {}).then(() => {
            this.checkReadyStatus()
        })
        this.mDB.executeSql(CREATE_RELROUTETASK_TABLE, {}).then(() => {
            this.checkReadyStatus()
        })
        this.mDB.executeSql(CREATE_USERS_TABLE, {}).then(() => {
            this.checkReadyStatus()
        })
        this.mDB.executeSql(CREATE_SCORE_TABLE, {}).then(() => {
            this.checkReadyStatus()
        })
        console.log('onCreate: Pushed all executeSql')
    }
    ...
    private checkReadyStatus() {
        ...
        if (this.pushedSQL === 0) {
            this.ready = true
            console.log("checkReadyStatus: READY!")
        } else {
            console.log("checkReadyStatus: not yet ready")
        }
    }

    onUpgrade(oldVersion: number, newVersion: number) {
        this.mDB.executeSql("DROP TABLE IF EXISTS " + DBC.DATABASE_TABLE_STATE, {})
        this.mDB.executeSql("DROP TABLE IF EXISTS " + DBC.DATABASE_TABLE_TASK, {})
        this.mDB.executeSql("DROP TABLE IF EXISTS " + DBC.DATABASE_TABLE_ROUTE, {})
        this.mDB.executeSql("DROP TABLE IF EXISTS " + DBC.DATABASE_TABLE_REL_ROUTE_TASK, {})
        this.mDB.executeSql("DROP TABLE IF EXISTS " + DBC.DATABASE_TABLE_USERS, {})
        this.mDB.executeSql("DROP TABLE IF EXISTS " + DBC.DATABASE_TABLE_SCORE, {})

        this.onCreate();
    }

    protected initTableVersions() {
     ...
     */
    // public Map<String, String> getTableVersions(){
    //     Map<String, String> result = new HashMap<String, String>();
    //     SQLiteDatabase db = DB_Handler.getInstance(context).getReadableDatabase();
    //     // SELECT option, value FROM mcm_state WHERE option = 'version'_task OR option = 'version_route' OR option = 'version_rel_route_task'
    //     Cursor cursor = db.query(DBC.DATABASE_TABLE_STATE, new String[]{DBC.DB_STATE.fields[1], DBC.DB_STATE.fields[2]}, "option = ? OR option = ? OR option = ?", new String[]{"version_task", "version_route", "version_rel_route_task"}, null, null, null);

    //     // If no rows are delivered - the init values are not inserted
    //     if(cursor.getCount() == 0){
    //         initTableVersions();
    //     }

    //     cursor = db.query(DBC.DATABASE_TABLE_STATE, new String[]{DBC.DB_STATE.fields[1], DBC.DB_STATE.fields[2]}, "option = ? OR option = ? OR option = ?", new String[]{"version_task", "version_route", "version_rel_route_task"}, null, null, null);

    //     cursor.moveToFirst();
    //     while(cursor.isAfterLast() == false){
    //         String option = cursor.getString(0);
