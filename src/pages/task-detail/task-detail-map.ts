...
import { ViewChild, ElementRef, OnDestroy, Component } from "@angular/core";
...
export class TaskDetailMap implements OnDestroy {
    ...
    constructor(
        ...
        for (
            let i = 0;
            i < parseInt(this.task.getSolutionGpsValue("points"));
            i++
        ) {
        ...
        switch (this.app.activeNarrative) {
            case "pirates":
                ...
                this.preDefinedPointIcon = L.icon({
                    iconUrl: "assets/icons/pirates/marker-task-gps-0.png",
                    iconSize: [50, 50],
                    iconAnchor: [25, 25],
                    className: "marker",
                });
                this.pointsIcons[0] = L.icon({
                    iconUrl: "./assets/icons/pirates/marker-task-gps-1.png",
                    iconSize: [50, 50],
                    iconAnchor: [25, 25],
                    className: "marker",
                });
                this.pointsIcons[1] = L.icon({
                    iconUrl: "./assets/icons/pirates/marker-task-gps-2.png",
                    iconSize: [50, 50],
                    iconAnchor: [25, 25],
                    className: "marker",
                });
                this.pointsIcons[2] = L.icon({
                    iconUrl: "./assets/icons/pirates/marker-task-gps-3.png",
                    iconSize: [50, 50],
                    iconAnchor: [25, 25],
                    className: "marker",
                });
                this.pointsIcons[3] = L.icon({
                    iconUrl: "./assets/icons/pirates/marker-task-gps-4.png",
                    iconSize: [50, 50],
                    iconAnchor: [25, 25],
                    className: "marker",
                });
            ...
            default:
                ...
                this.preDefinedPointIcon = L.icon({
                    iconUrl: "./assets/icons/marker-task-gps-0.png",
                    iconSize: [50, 50],
                    iconAnchor: [25, 50],
                    className: "marker",
                });
                this.pointsIcons[0] = L.icon({
                    iconUrl: "./assets/icons/marker-task-gps-1.png",
                    iconSize: [50, 50],
                    iconAnchor: [25, 50],
                    className: "marker",
                });
                this.pointsIcons[1] = L.icon({
                    iconUrl: "./assets/icons/marker-task-gps-2.png",
                    iconSize: [50, 50],
                    iconAnchor: [25, 50],
                    className: "marker",
                });
                this.pointsIcons[2] = L.icon({
                    iconUrl: "./assets/icons/marker-task-gps-3.png",
                    iconSize: [50, 50],
                    iconAnchor: [25, 50],
                    className: "marker",
                });
                this.pointsIcons[3] = L.icon({
                    iconUrl: "./assets/icons/marker-task-gps-4.png",
                    iconSize: [50, 50],
                    iconAnchor: [25, 50],
                    className: "marker",
                });
    ...
    setMarker(index: number) {
        ...
        if (location != null) {
            let locationLatLng = new L.LatLng(
            ...
            if (this.pointMarkers[index] == null) {
                ...
                let newMarker = L.marker(locationLatLng, {
                    icon: this.pointsIcons[index],
                });
    ...
    markerCanBeSet(clickLatLng: L.LatLng): boolean {
         ...
         */
        let distanceToTask = (L as any).GeometryUtil.length([
            new L.LatLng(this.taskDetails.lat, this.taskDetails.lon),
            clickLatLng,
        ]);
        if (distanceToTask > this.ALLOWED_DISTANCE_TO_TASK) {
            ...
            if (this.taskDetails.getSolutionGpsValue("task") == "linearFx") {
                ...
                let xy = (L as any).GeometryUtil.destination(
                    this.axisPoints.x,
                    (L as any).GeometryUtil.bearing(
                        this.axisPoints.origin,
                        this.axisPoints.y,
                    ),
                    this.AXIS_LENGTH,
                );

                let closestOnX = (L as any).GeometryUtil.closestOnSegment(
                    this.map,
                    clickLatLng,
                    this.axisPoints.origin,
                    this.axisPoints.x,
                );
                let closestOnY = (L as any).GeometryUtil.closestOnSegment(
                    this.map,
                    clickLatLng,
                    this.axisPoints.origin,
                    this.axisPoints.y,
                );
                let closestOnXY = (L as any).GeometryUtil.closestOnSegment(
                    this.map,
                    clickLatLng,
                    this.axisPoints.x,
                    xy,
                );
                let closestOnYX = (L as any).GeometryUtil.closestOnSegment(
                    this.map,
                    clickLatLng,
                    this.axisPoints.y,
                    xy,
                );
                let distanceX = (L as any).GeometryUtil.length([
                    closestOnX,
                    clickLatLng,
                ]);
                let distanceY = (L as any).GeometryUtil.length([
                    closestOnY,
                    clickLatLng,
                ]);
                let distanceXY = (L as any).GeometryUtil.length([
                    closestOnXY,
                    clickLatLng,
                ]);
                let distanceYX = (L as any).GeometryUtil.length([
                    closestOnYX,
                    clickLatLng,
                ]);
                if (
    ...
    insertPreDefinedPoints(points: Array<Array<number>>) {
        for (let i = 0; i < points.length; i++) {
            let preDefinedPoint = L.marker(
                new L.LatLng(points[i][0], points[i][1]),
                { icon: this.preDefinedPointIcon },
            );
    ...
    insertAxis(origin: Array<number>, dPoint: Array<number>) {
        ...
        bCoor = (L as any).GeometryUtil.destination(
            aCoor,
            bearingAB,
            this.AXIS_LENGTH,
        ); // Override bCoor with a point that is 100 meter in right direction
        /*
        if (disAB < this.AXIS_LENGTH) {
            bCoor = (L as any).GeometryUtil.destination(aCoor, bearingAB, this.AXIS_LENGTH);
        }
        */
        let yCoor = (L as any).GeometryUtil.destination(
            aCoor,
            bearingAB - 90,
            this.AXIS_LENGTH,
        );
        let xArrowUp = (L as any).GeometryUtil.destination(
            bCoor,
            bearingAB - 180 + this.ARROW_DEGREE,
            this.ARROW_LENGTH,
        );
        let xArrowDown = (L as any).GeometryUtil.destination(
            bCoor,
            bearingAB + 180 - this.ARROW_DEGREE,
            this.ARROW_LENGTH,
        );
        let yArrowUp = (L as any).GeometryUtil.destination(
            yCoor,
            bearingAB - 90 - 180 + this.ARROW_DEGREE,
            this.ARROW_LENGTH,
        );
        let yArrowDown = (L as any).GeometryUtil.destination(
            yCoor,
            bearingAB - 90 + 180 - this.ARROW_DEGREE,
            this.ARROW_LENGTH,
        );
        ...
        (L as any)
            .marker(
                (L as any).GeometryUtil.destination(
                    bCoor,
                    bearingAB,
                    this.MARK_LENGTH * 3,
                ),
                {
                    ...
                    icon: this.getLabeledIcon("X", "axis-label", "x"),
                },
            )
            .addTo(this.map);
        (L as any)
            .marker(
                ...
                {
                    rotationAngle: bearingAB - 90,
                    icon: this.getLabeledIcon("Y", "axis-label", "y"),
                },
            )
            .addTo(this.map);
        ...
        for (let i = 1; i < this.AXIS_LENGTH / this.MARK_DISTANCE; i++) {
            ...
            let coordOnXAxis = (L as any).GeometryUtil.destination(
                aCoor,
                bearingAB,
                this.MARK_DISTANCE * i,
            );
            let coordOnYAxis = (L as any).GeometryUtil.destination(
                aCoor,
                bearingAB - 90,
                this.MARK_DISTANCE * i,
            );
            let innerPointX = (L as any).GeometryUtil.destination(
                coordOnXAxis,
                bearingAB - 90,
                this.MARK_LENGTH,
            );
            let outerPointX = (L as any).GeometryUtil.destination(
                coordOnXAxis,
                bearingAB - 90,
                -this.MARK_LENGTH,
            );
            let innerPointY = (L as any).GeometryUtil.destination(
                coordOnYAxis,
                bearingAB,
                this.MARK_LENGTH,
            );
            let outerPointY = (L as any).GeometryUtil.destination(
                coordOnYAxis,
                bearingAB,
                -this.MARK_LENGTH,
            );
            L.polyline([outerPointX, innerPointX], {
                color: "red",
                weight: markerWidth,
                opacity: 0.7,
            }).addTo(this.map);
            L.polyline([outerPointY, innerPointY], {
                color: "red",
                weight: markerWidth,
                opacity: 0.7,
            }).addTo(this.map);
            ...
            if (i == 5 || i == 10) {
                let xLabelCoord = (L as any).GeometryUtil.destination(
                    coordOnXAxis,
                    bearingAB + 90,
                    this.MARK_LENGTH * 3,
                );
                let yLabelCoord = (L as any).GeometryUtil.destination(
                    coordOnYAxis,
                    -bearingAB,
                    this.MARK_LENGTH * 3,
                );
    ...
    insertLinearFxGraph() {
        if (this.areAllPointsSet()) {
            ...
            let pointA = (L as any).GeometryUtil.destination(
                points[0],
                bearing,
                -1 * this.LINEAR_FX_EXTEND,
            );
            let pointB = (L as any).GeometryUtil.destination(
                points[1],
                bearing,
                this.LINEAR_FX_EXTEND,
            );
            this.linearFxGraph = L.polyline([pointA, pointB], {
                color: "blue",
            }).addTo(this.map);
        }
    }
    ...
    loadMap() {
        ...
        let subDomains = this.route.getTilesServerSubdomains(
        ...
        if (this.map == null) {
            ...
            let zoomLevels = Helper.calculateZoom(
                this.route.getViewBoundingBoxLatLng(),
            );
            tilesDb.initialize().then(() => {
                console.log("Tiles DB Initialized");
                let offlineLayer = (L.tileLayer as any).offline(
                    mapquestUrl,
                    tilesDb,
                    {
            ...
            this.gpsService
                ...
                .then((resp) => {
                    if (resp && resp.coords) {
                        ...
                        this.watchSubscription = this.gpsService
                            .watchPosition()
                            .subscribe((resp) => {
                                if (resp && resp.coords) {
                                    const lanlng = new L.LatLng(
