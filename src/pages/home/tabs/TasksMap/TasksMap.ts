...
export class TasksMap implements OnDestroy {
    ...
    private sessionTimeTimer = Observable.interval(
        TasksMap.UPDATE_SESSION_TIME_INTERVAL_IN_SECS * 1000,
    );
    ...
    isTrailCompleted() {
        if (this.route.isAnswerFeedbackEnabled()) {
            return (
                this.scoreTaskList &&
                this.score.getTasksSolved().length +
                    this.score.getTasksSolvedLow().length +
                    this.score.getTasksFailed().length ==
                    this.scoreTaskList.length
            );
        } else {
            return (
                this.score.getTasksSaved() &&
                this.score.getTasksSaved().length == this.scoreTaskList.length
            );
        }
    }
            ...
            MCMTrailFinishedModal,
            {
                ...
                callback: function () {
                    modal.dismiss().then(async () => {
                        ...
                        that.route.completedDate = new Date()
                            .toDateString()
                            .split(" ")
                            .slice(1)
                            .join(" ");
                        let regex = /.*(\d+.\d{1,2})/gm;
                        await that.progressService.increaseProgressCounter(
                            ProgressCounter.DISTANCE,
                            parseFloat(regex.exec(that.route.length)[1]),
                        );
                        await that.routeApiService.updateDownloadedRoute(
                            that.route,
                        );
                    });
                },
    ...
    async initView() {
        ...
        this.score = (
            await this.routeApiService.getDetailsForRoute(this.route)
        ).score;
        ...
        if (this.sessionInfo != null) {
            ...
            if (this.sessionInfo.started === false) {
                ...
                if (this.sessionInfo.sessionUser.assigned_task_id != 0) {
                    ...
                    await this.forceStartFromTask(
                        this.sessionInfo.sessionUser.assigned_task_id,
                    );
                ...
                await this.chatAndSessionService.updateSession(
                    this.sessionInfo,
                );
        ...
        if (this.navParams.data.tasksMapState) {
            ...
            if (
                this.taskToSkip ||
                (this.state.selectedStartTask &&
                    (this.score
                        .getTasksSolved()
                        .indexOf(this.state.selectedTask.id) > -1 ||
                        this.score
                            .getTasksSolvedLow()
                            .indexOf(this.state.selectedTask.id) > -1))
            ) {
            ...
            if (!this.state) {
                ...
                if (this.state.selectedTask) {
                    this.state.visibleTasks[this.state.selectedTask.position] =
                        true;
    ...
    private showGuidedTrailModalWithDelay(delay) {
        ...
        setTimeout(function () {
            that.modalsService.showDialog(
                "a_guided_trail_title",
                "a_guided_trail",
                "no",
                () => {},
                "yes",
    ...
    private async forceStartFromTask(taskId) {
        ...
        if (selectedTask.inactive) {
            ...
            let index = this.mapTaskList.findIndex(
                (task) => selectedTask.id === task.id,
            );
            let nextTaskIndex =
                index + 1 < this.mapTaskList.length ? index + 1 : 0;
    ...
    initializeMapHandler() {
        if (!this.mapHandler) {
            if (this.route.isMapAvailableOffline()) {
                this.mapHandler = new LeafletMapHandler(
                    "tasks-map",
                    this.route,
            ...
            this.mapHandler = new MapboxMapHandler(
                "tasks-map",
                this.route,
                this.app.activeNarrative,
            );
    ...
    updateSession(sessionInfo: SessionInfo) {
        ...
        if (sessionInfo && sessionInfo.session) {
            if (this.routeId != sessionInfo.session.trail_id) {
                ...
                this.sessionTime();
                this.sessionTimeSubscription = this.sessionTimeTimer.subscribe(
                    (tick) => {
                        this.sessionTime();
                    },
                );
    ...
    async loadMap() {
        ...
        this.taskClickSubscription = this.mapHandler.taskClickedEvent.subscribe(
            (taskId) => {
        ...
        this.watchSubscription = this.gpsService
            ...
            .subscribe((position) => {
                if (position?.coords) {
                    this.mapHandler.updateUserPosition(
    ...
    async redrawMarkers() {
        ...
        await this.refreshTaskLists();
        return this.mapHandler.redrawMarkers(
            this.mapTaskList,
            this.state,
            this.score,
        );
    ...
    async selectStartPoint() {
        ...
        return this.modalsService.presentTaskListModal(
            this.route,
            this.score,
            this.state,
            this.app.activeNarrative,
            ...
            function (selectedTask: Task) {
                ...
                if (that.sessionInfo != null) {
                    let details = JSON.stringify({
                        title: that.state.selectedTask.title,
                    });
                    that.chatAndSessionService.addUserEvent(
                        "event_trail_start_from_task",
                        details,
                        that.state.selectedTask.id.toString(),
                    );
                }
            },
        );
    ...
    displayResetTasksModal() {
        this.modalsService.showDialog(
            "a_route_detail_settings_resetTasks",
            "a_route_detail_settings_resetTasks_msg",
            "no",
            () => {},
            "yes",
            async () => {
    ...
    resetTasks() {
        return new Promise<void>((resolve) => {
            this.routeApiService
                .resetScoreForRoute(this.route.code)
                .then(async () => {
                    ...
                    if (
                        this.sessionInfo != null &&
                        this.sessionInfo.sessionUser.assigned_task_id != 0
                    ) {
                        await this.forceStartFromTask(
                            this.sessionInfo.sessionUser.assigned_task_id,
                        );
                    ...
                    await this.routeApiService.updateDownloadedRoute(
                        this.route,
                    );
                    await this.redrawMarkers();
                    resolve();
    ...
    async sessionFinished() {
        this.modalsService.showDialog(
            "a_private_session_quit",
            "a_private_session_quit_text",
            "no",
            () => {},
            "yes",
            () => {
                ...
                buttons: [
                    {
                        ...
                        callback: function () {
                            modal.dismiss();
                            let finishedModal = that.modalCtrl.create(
                                MCMSessionFinishedModal,
                                {
    ...
    private sessionTime() {
        ...
        if (
            currentTimeUnix > startTimeInUnix - 3600 &&
            currentTimeUnix < endTimeInUnix
        ) {
            this.startInterval = true;
            if (
                currentTimeUnix < startTimeInUnix &&
                currentTimeUnix < endTimeInUnix
            ) {
    ...
    showIntroModal(): Promise<void> {
        return new Promise<void>((success) => {
            ...
            let introModal = this.modalCtrl.create(
                MCMIntroModal,
                {
                    title: title,
                    message: message,
    ...
    async refreshTaskLists() {
        if (this.sessionInfo != null) {
            ...
            // filter out submission task if we have no session
            this.mapTaskList = (
                await this.routeApiService.getDetailsForRoute(this.route)
            ).tasks.filter((t) => t.solutionType != "submission");
        }

        let scoredTasks = this.mapTaskList.filter((task) => {
            return task.taskFormat !== TaskFormat.GROUP;
        });
        const groups = this.mapTaskList.filter((task) => {
            return task.taskFormat === TaskFormat.GROUP;
        });
        for (let group of groups) {
            scoredTasks = scoredTasks.concat(group.getLegitSubtasks());
        }
        scoredTasks = scoredTasks.filter((task) => {
