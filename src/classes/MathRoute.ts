...
export class MathRoute {
  ...
  protected calcBoundingBoxAndCenter() {
    const padding: number = 0.0015
    const jsonBB = JSON.parse(this.getInfo("bounding_box"))
    const jsonCenter = JSON.parse(this.getInfo("center"))
    ...
    this.boundingBox = L.latLngBounds([[north, east], [south, west]])
    this.center = L.latLng([jsonCenter[0], jsonCenter[1]])
  }
  //     /*
  ...
  initTasks(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      if (this.tasks.length == 0) {
        ...
        const dbh = DB_Handler.getInstance()
        dbh.getRouteTaskIds(this.Id).then(taskIds => {
          console.log(`Returned ${taskIds.length} tasks.`)

          let promises = []

          taskIds.forEach(taskId => {
            promises.push(
              dbh.getMathTaskById(+taskId).then(mt => {
                if (mt != null) {
                  this.addTask(mt)
                  console.log(`Add task ${mt.Id}`)
                }
              }).catch(error => {
                console.error(`initTasks: Error: ${JSON.stringify(error)}`)
                reject(error)
              })
            )
          })
        ...
        console.log(`WE HAVE TASKS ${this.tasks.length}`)
        resolve()
      }
    })
  }
