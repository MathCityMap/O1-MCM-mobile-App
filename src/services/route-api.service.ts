...
export class RouteApiService {
    ...
    async fetchPublicRoutes(count = 20) {
        ...
        if (!this.position) {
            ...
            } = this.gpsService.getLastPosition();
            if (!lastPosition) {
                lastPosition = await this.gpsService.getCurrentPosition();
                if (!lastPosition) {
                    console.debug(
                        "Could not determine position, using Frankfurt am Main as fallback",
                    );
                    lastPosition = {
                        coords: {
                            longitude: 8.684708439135116,
                            latitude: 50.11099446102349,
                        },
                    };
                }
            }
        ...
        let newRoutes = [];
        try {
            newRoutes = await this.internalfetchRoutesForPosition(
    ...
    async updateDownloadedRoutes(): Promise<void> {
        ...
        for (let route of downloaded) {
            try {
                let updatedRoute = await this.internalFetchRouteForUnlock(
                    route.code,
                );
    ...
    async findRouteByCode(code: string): Promise<Route> {
        ...
        if (!route) {
            ...
            if (!route) {
                route = this._unlockedRoutes.find(
                    (uRoute) => uRoute.code === code,
                );
                if (!route) {
                    try {
    ...
    unlockRoute(route: Route) {
        let index = this._unlockedRoutes.findIndex(
            (uRoute) => uRoute.id === route.id,
        );
        if (index === -1) {
    ...
    async getDetailsForRoute(route: Route): Promise<RouteInfos> {
        let details: RouteInfos = await this.storage.get(
            DOWNLOADED_ROUTE_INFOS_PREFIX + route.code,
        );
        if (!details) {
            return { tasks: [], score: new Score() };
        }
        return {
            tasks: Task.convertGenericsToTaskArray(details.tasks),
            score: Score.fromGenericScore(details.score),
        };
    }
    ...
    async getTaskDetails(
        ...
        taskId: number,
        parentId?: number,
    ): Promise<{ task: Task; score: Score }> {
        let details: RouteInfos = await this.storage.get(
            DOWNLOADED_ROUTE_INFOS_PREFIX + routeCode,
        );
        if (!details || !details.tasks) {
            return { task: null, score: new Score() };
        }
        let parent;
        if (parentId) {
            parent = details.tasks.find((dTask) => dTask.id === parentId);
        }
        let taskList = parent ? parent.subtasks : details.tasks;
        if (!taskList) {
            return { task: null, score: Score.fromGenericScore(details.score) };
        }
        let task = taskList.find((dTask) => dTask.id === taskId);
        return {
            task: Task.fromGenericTask(task),
            score: Score.fromGenericScore(details.score),
        };
    }
        ...
        try {
            ...
            if (!route.isNarrativeEnabled()) {
                if (route.isMapAvailableOffline()) {
                    ...
                    try {
                        await this.imagesService.downloadAndUnzip(
                            route,
                            (done) => {
                ...
                let zoomLevels = Helper.calculateZoom(
                    route.getViewBoundingBoxLatLng(),
                );
                await CacheManagerMCM.downloadTiles(
                    route,
                    zoomLevels.min_zoom,
                    zoomLevels.max_zoom,
                    (done, total, url) => {
            ...
            await this.imagesService.downloadURLs(
                downloadImages,
                false,
                (done, total, url) => {
                    alreadyDownloadedUrls.push(url);
            ...
            await this.imagesService.removeDownloadedURLs(
    ...
    async updateRouteInfos(route: Route) {
        ...
        try {
            let tasks = await this.internalFetchDetailsForRoute(
                route.code,
                true,
            );
    ...
    async removeDownloadedRoute(route: Route) {
        ...
        if (routeInfos) {
            this.imagesService.removeDownloadedURLs(
                this.getDownloadImagesForTasks(routeInfos.tasks),
                false,
            );
        }
    ...
    public async updateDownloadedRoute(route: Route) {
        let downloadedRoutes = await this.getDownloadedRoutes();
        let indexToPatch = downloadedRoutes.findIndex(
            (dRoute) => dRoute.id === route.id,
        );
        if (indexToPatch !== -1) {
    ...
    public async updateScoreForRoute(score: Score, routeCode: string) {
        let stats: RouteInfos = await this.storage.get(
            DOWNLOADED_ROUTE_INFOS_PREFIX + routeCode,
        );
        stats.score = score;
        await this.storage.set(
    ...
    async insertOrUpdateTaskState(
        score: Score,
        detailsToSave: TaskState,
        routeCode: string,
    ) {
    ...
    private async removeRouteFromDownloadedList(route: Route) {
        ...
        let downloadedRoutes = await this.getDownloadedRoutes();
        downloadedRoutes = downloadedRoutes.filter(
            (dRoute) => dRoute.id !== route.id,
        );
    ...
    private getDownloadImagesForTasks(tasks: Task[]) {
        let result = [];
        tasks
            .map((task) => task.getImagesForDownload())
            .map((images) => {
    ...
    private getTileURLs(route: Route) {
        ...
        return CacheManagerMCM.getTileURLs(
            route,
            zoomLevels.min_zoom,
            zoomLevels.max_zoom,
        );
    ...
    private async internalfetchRoutesForPosition(
        lat: number,
        lon: number,
        offset = 0,
        limit = 20,
            ...
            .pipe(
                map((value) => {
                    ...
                    return value.items.map((routeResponse) => {
                        let downloadedRoute = downloadedRoutes.find(
                            (dRoute) => routeResponse._id === dRoute.id,
                        );
                        if (downloadedRoute) {
                            let updatedRoute =
                                Route.fromRouteResponse(routeResponse);
                            updatedRoute.downloaded = true;
                            updatedRoute.downloadedDate =
                                downloadedRoute.downloadedDate;
    ...
    private async internalFetchDetailsForRoute(
        code: string,
    ...
    private updateRouteInPublicList(routeToUpdate: Route) {
        if (this._publicRoutes) {
            let index = this._publicRoutes.findIndex(
    ...
    public async migrateDataFromSQLiteStorage() {
        ...
        try {
            ...
            for (let route of routes) {
                let tasks = (await route.getTasks()).map((task) => {
                    task.forceSupportTask = Boolean(
                        route.isSubtaskRequired(task.id),
                    );
                    task.s3Media = {
                        image: RouteApiService.createS3ImageMediaForUrl(
                            task.image,
                        ),
                    };
                    if (task.getHint(1).type === "image") {
                        task.s3Media.hint1 =
                            RouteApiService.createS3ImageMediaForUrl(
                                task.getHint(1).value,
                            );
                    }
                    if (task.getHint(2).type === "image") {
                        task.s3Media.hint2 =
                            RouteApiService.createS3ImageMediaForUrl(
                                task.getHint(2).value,
                            );
                    }
                    if (task.getHint(3).type === "image") {
                        task.s3Media.hint3 =
                            RouteApiService.createS3ImageMediaForUrl(
                                task.getHint(3).value,
                            );
                    }
                    if (task.getSolutionSampleImgSrc() !== "") {
                        task.s3Media.solutionsample =
                            RouteApiService.createS3ImageMediaForUrl(
                                task.getSolutionSampleImgSrc(),
                            );
                    }
                    return task;
                });
            ...
            console.warn(
                "Migration of old data failed, probably because there was no data",
                e,
            );
        }
        ...
        await this.ormService
            .removeAllSqLiteData()
            .catch((e) => console.warn("failed to delete old sqlite data", e));
        await this.storage.set(key, true);
    }

    private static createS3ImageMediaForUrl(url: string) {
        let imageUrlSegments = url.split("/");
        let frontParts = imageUrlSegments.splice(
            0,
            imageUrlSegments.length - 1,
        );
        return {
            mimeType: "image/*",
